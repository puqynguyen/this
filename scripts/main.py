...
def main():
    ...
    for stock_symbol in stock_symbols:
        ...
        split_date = end_date - timedelta(days=60)  # Chừa 60 ngày cuối để xác định current day
        ...
        train_data, val_data = process_data(stock_symbol, price_data, news_data, start_date, end_date, split_date)
        ...
        print(f"Đã lưu dữ liệu validation vào {DATA_DIR}/processed_data_{stock_symbol}_val.csv")

        # Bước 3: Huấn luyện và đánh giá mô hình
        print(f"Đang huấn luyện và đánh giá mô hình cho {stock_symbol}...")
        model = train_model(stock_symbol, train_data, val_data)
        if model is None:
